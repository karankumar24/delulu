// Parsing a transcript into events, and tolerating what it cannot read.
...
describe('parseSessionLog', () => {
  ...
  it('leaves out subagent turns, so their work is not credited to the main agent', () => {
    const log = tmpLog(
      'sidechain.jsonl',
      [
        '{"type":"summary","sessionId":"s1"}',
        // A subagent turn: its text, tool_use and tool_result are all dropped.
        JSON.stringify({
          type: 'assistant',
          isSidechain: true,
          message: {
...
describe("parseSessionLog: the user's messages", () => {
  ...
  // The image block contributes no text of its own.
...
describe('parseSessionLog: messages typed while the agent worked', () => {
...
describe('parseSessionLog: who sent a queued message', () => {
...
describe('queued messages: a twin is adjacent, a repeat is not', () => {
  ...
  it('keeps the same words sent again much later', () => {
