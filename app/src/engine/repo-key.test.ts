...
describe('repoKey', () => {
  ...
  it('resolves to the top-level when git refuses on dubious ownership — the bug: handoff orphaned in a subdir', () => {
    ...
    withEnv({ GIT_TEST_ASSUME_DIFFERENT_OWNER: '1' }, () => {
      // The switch is git's own, and not every build honours it: a CI runner working as the
      // directory's owner, or a git configured with a blanket `safe.directory`, refuses nothing.
      // Where it does not refuse there is no dubious-ownership path to exercise, so the test says
      // so and stops rather than asserting on a git that never entered the branch under test.
      let refuses = false;
      try {
