...
describe('handoff — --restate can never destroy a sealed handoff', () => {
  ...
  it('ALLOWS the reseal when one transcript reaches the two runs under two names', () => {
    ...
    // The base is resolved FIRST, so `stored` below is canonical on every platform. Both names the
    // transcript arrives under are then symlink routes to it: reaching the file only through links
    // is what makes the two strings differ on Linux too, where `/tmp` is a real directory and the
    // earlier version of this test compared a path to itself and passed vacuously.
    scratch = realpathSync(mkdtempSync(join(tmpdir(), 'delulu-handoff-')));
    ...
    const canonical = join(scratch, 'real', 'session.jsonl');
    const real = join(scratch, 'alias', 'session.jsonl');
    const linked = join(scratch, 'link', 'session.jsonl');
    writeFileSync(canonical, typed('alpha one: keep the parser exactly as we discussed') + '\n');
