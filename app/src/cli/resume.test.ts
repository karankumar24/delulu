...
describe('resume — a handoff too big for one output is still delivered whole', () => {
  const big = (): string => {
    ...
    const base = join(dir, '.delulu-handoff', '2026-01-01T00-00-01');
    mkdirSync(base, { recursive: true });
    // 6 x 5200 = 31,200 bytes of body ALONE, against a 27,000-byte budget, so this fixture is over
    // the line on its own. It used to be 4,200 each: 25,200, which only exceeded the budget once
    // delulu's own preamble was added on top. That made the test a hostage to the LENGTH OF THE
    // PROSE, and it duly broke when the preamble lost a character per em-dash. A trim test must be
    // decided by the payload, never by the wording of the notice printed above it.
    const bulk = 'x'.repeat(5200);
    writeFileSync(join(base, 'payload.md'),
      `# h\n\n## ${SECTION.state}\n- Branch \`main\`\n\n---\n> **Everything below** is the agent.\n\n`
      + `## ${SECTION.thread}\n${bulk}\n\n## ${SECTION.elseWrong}\n${bulk}\n\n## ${SECTION.broke}\n${bulk}\n\n`
