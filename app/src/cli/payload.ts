...
/** Every heading delulu itself writes, LONGEST first so a name that extends another wins. */
const KNOWN_HEADINGS: readonly string[] = [
  ...new Set(Object.values(SECTION).flatMap((n) => headingsFor(n))),
].sort((a, b) => b.length - a.length);

/**
 * The section a heading LINE belongs to, by identity rather than by punctuation.
 *
 * Headings are `## <name><subtitle>`, and the subtitle is prose that changes with the payload
 * ("every message you sent" against "the last 12 of your 40"). Callers need the stable half.
 *
 * This used to be `head.split(' — ')[0]`, which made one punctuation mark load-bearing in the
 * FORMAT: the separator could not change without stranding every sealed handoff, and two headings
 * that OPENED with the separator (`## — alpha`, `## — beta`) both keyed to `##` and silently
 * merged, which `verifiedSections` documented as a live collision. Resolving against the names
 * delulu actually writes removes both problems at once. It reads OLD handoffs unchanged, because
 * the name is the prefix in either spelling, and it needs no compatibility branch to do it.
 *
 * A heading delulu did not write resolves to itself, trimmed, so hand-written sections stay
 * distinct from one another instead of collapsing onto a shared separator.
 */
export function sectionOf(head: string): string {
 ...
 */
// Both separators, because a SEALED handoff is never rewritten: everything captured before
// delulu stopped writing em-dashes still carries one here, and dropping it from the pattern
// would silently stop those rules being recognised as justified. The written form is the comma.
