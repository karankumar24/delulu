...
The `delulu` command lives on `PATH` inside Claude Code's Bash tool, which is where the two slash commands run it. Nothing is installed into your own shell, so typing `delulu` in your terminal answers `command not found` — that is expected, not a failed install.
